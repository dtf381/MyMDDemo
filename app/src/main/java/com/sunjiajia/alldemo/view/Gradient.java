...
/**
 * Created by mk on 2016/12/28.
 * 图片切换，渐变切换
 *
 * 这个控件的使用非常简单只要在布局文件中使用我们自定义的控件,然后调用setTime设置动画切换的时间,
 * setListener设置图片的点击事件,setImagevies设置图片就可以实现效果.考虑到内存泄漏的问题,只要在ondestry方法里面调用stop方法即可.
 */
    ...
    public void cratePoint() {
        if (null != imageViews && imageViews.size() > 0) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                viewPoint.setBackgroundResource(R.drawable.img_mask);
